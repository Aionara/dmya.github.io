...
app.callSuffix = ' 到办公室';
// 控制面板折叠状态
app.controlsCollapsed = false;
// 头部控制折叠状态
app.headerCollapsed = false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initParticles();
    updateDisplay();
    // Set initial mode panels visibility (ensure callMessageOptions respects currentMode)
    try { switchMode(app.currentMode); } catch (e) { /* ignore if elements not ready */ }
    // 教程弹窗初始化：绑定ESC与遮罩点击
    setupTutorialModal();
});
...
function loadData() {
    ...
    if (savedSettings) {
        ...
        app.callPrefix = settings.callPrefix ?? app.callPrefix;
        app.callSuffix = settings.callSuffix ?? app.callSuffix;
        app.controlsCollapsed = settings.controlsCollapsed ?? false;
        app.headerCollapsed = settings.headerCollapsed ?? false;
    }
...
function saveData() {
    ...
    localStorage.setItem('settings', JSON.stringify({
        ...
        ,
        callPrefix: app.callPrefix,
        callSuffix: app.callSuffix,
        controlsCollapsed: app.controlsCollapsed
        ,headerCollapsed: app.headerCollapsed
    }));
}
...
function switchMode(mode) {
    ...
    // 叫号文本自定义面板：仅当处于上课提问（question）模式时显示
    const callMsgPanel = document.getElementById('callMessageOptions');
    if (callMsgPanel) {
        callMsgPanel.style.display = (mode === 'question') ? 'block' : 'none';
    }
...
function initEventListeners() {
    ...
    // 初始化叫号文本自定义控件
    const prefixInput = document.getElementById('callPrefixInput');
    const suffixInput = document.getElementById('callSuffixInput');
    const preview = document.getElementById('previewCallMsg');
    const saveBtn = document.getElementById('saveCallMsgBtn');

    if (prefixInput && suffixInput && preview) {
        // 填充保存的值
        prefixInput.value = app.callPrefix || '';
        suffixInput.value = app.callSuffix || '';
        preview.textContent = `${app.callPrefix || ''}张三${app.callSuffix || ''}`;

        const updatePreview = () => {
            preview.textContent = `${prefixInput.value || ''}张三${suffixInput.value || ''}`;
        };
        ...
        saveBtn && saveBtn.addEventListener('click', () => {
            app.callPrefix = prefixInput.value;
            app.callSuffix = suffixInput.value;
            saveData();
            showToast('叫号文本已保存');
        });
    }
    ...
    function updateControlsCollapsedUI() {
        if (!controlsEl) return;
        if (app.controlsCollapsed) {
            controlsEl.classList.add('collapsed');
            if (controlsToggleBtn) {
                controlsToggleBtn.setAttribute('aria-expanded', 'false');
                // 设置为滑动开关
                controlsToggleBtn.innerHTML = '<span class="switch-track"><span class="switch-thumb"></span></span><span class="switch-label">展开控制面板</span>';
            }
        } else {
            ...
            if (controlsToggleBtn) {
                ...
                // 设置为滑动开关
                controlsToggleBtn.innerHTML = '<span class="switch-track"><span class="switch-thumb"></span></span><span class="switch-label">收起控制面板</span>';
            }
        }
    }
    ...
    if (controlsToggleBtn) {
        controlsToggleBtn.addEventListener('click', () => {
            app.controlsCollapsed = !app.controlsCollapsed;
            updateControlsCollapsedUI();
            saveData();
        });
    }
    ...
    // 头部控制折叠逻辑
    const headerToggleBtn = document.getElementById('headerControlsToggle');
    const headerControlsGroup = document.getElementById('headerControlsGroup');
    function updateHeaderControlsUI() {
        if (!headerControlsGroup) return;
        if (app.headerCollapsed) {
            headerControlsGroup.classList.add('collapsed');
            if (headerToggleBtn) {
                headerToggleBtn.setAttribute('aria-expanded', 'false');
                // 折叠时显示小型滑动开关（左）
                headerToggleBtn.innerHTML = '<span class="switch-track"><span class="switch-thumb"></span></span><span class="switch-label">展开</span>';
            }
        } else {
            headerControlsGroup.classList.remove('collapsed');
            if (headerToggleBtn) {
                headerToggleBtn.setAttribute('aria-expanded', 'true');
                // 展开时显示小型滑动开关（右）并文字提示收起
                headerToggleBtn.innerHTML = '<span class="switch-track"><span class="switch-thumb"></span></span><span class="switch-label">收起</span>';
            }
        }
    }
...
function callStudent(name) {
    ...
    const display = document.getElementById('displayName');
    if (display) {
        display.textContent = `${app.callPrefix || ''}${name}${app.callSuffix || ''}`;
    }
    ...
    if (app.speechEnabled) {
        try {
            ...
            speak(`${app.callPrefix || ''}${name}${app.callSuffix || ''}`);
        } catch (e) { /* ignore */ }
    }
